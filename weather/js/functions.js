...
sessStore.setItem("latLong",latLong);
      ...
      sessStore.setItem("Tempurature",temperature);
      sessStore.setItem("High Temp",highTemp);
      sessStore.setItem("Low Temp",lowTemp);
      ...
      sessStore.setItem("Wind Speed",windSpeed);
      sessStore.setItem("Wind Gust",windGust);
...
lowTemp.innerHTML = sessStore.getItem('lowTemp') + "°F";
...
//Build the Conditions Component
//As with the wind component, the conditions component will use much of the same code, except we will build an <img> element to display the icons for weather conditions.

//You will probably have to set up a CSS rule to display the images smaller to fit the component. My rule looks like this:

/* Constrain condition images */
  #curCond img{
  max-width: 40%;
